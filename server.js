// Cartback - abandoned checkout recovery via Vapi + Twilio SMS
// Complete file with OAuth callback handler and SMS support
...
async function sendSms(phone, body) {
  ...
  if (MODE === 'test' && !ALLOWED_PHONES.includes(phone)) {
    return { success: false, error: 'phone not in allowed list (test mode)' };
  }
  try {
...
async function maybeSendSms(rec, reason) {
  if (!rec.customer.phone || !rec.recoveryUrl) {
    addEvent(rec, 'sms_skipped', 'no phone or recovery URL');
    return;
  }
  const firstName = (rec.customer.name || '').split(' ')[0] || 'there';
  const body = 'Hi ' + firstName + ', it\'s ' + STORE_NAME + '. Here\'s the link to your cart: ' + rec.recoveryUrl + '. Reply STOP to opt out.';
  const result = await sendSms(rec.customer.phone, body);
  if (result.success) {
    rec.smsSent = true;
    rec.smsSid = result.sid;
    addEvent(rec, 'sms_sent', reason + ' - ' + result.sid);
  } else {
    addEvent(rec, 'sms_failed', reason + ' - ' + result.error);
  }
}
...
app.get('/test-call/:secret', function(req, res) {
  ...
  const rec = {
    ...
    recoveryUrl: 'https://example.com/checkouts/test',
    status: 'pending_call',
    events: [],
    timer: null,
    scheduledFor: null,
...
// Test SMS endpoint
app.get('/test-sms/:secret', async function(req, res) {
  if (!STATS_SECRET || req.params.secret !== STATS_SECRET) return res.status(404).send('not found');
  const phone = ALLOWED_PHONES[0];
  if (!phone) return res.send('No phone in ALLOWED_PHONES');
  const body = 'Test from ' + STORE_NAME + '. If you got this, SMS is working. Reply STOP to opt out.';
...
app.post('/webhooks/checkouts', function(req, res) {
  ...
  if (!rec) {
    ...
    if (checkout.abandoned_checkout_url) rec.recoveryUrl = checkout.abandoned_checkout_url;
  }
  ...
  if (!rec.customer.phone) {
    ...
    if (latest) {
      ...
      if (apiPhone) {
        ...
        if (latest.abandoned_checkout_url) rec.recoveryUrl = latest.abandoned_checkout_url;
...
// Pull the agreed_to_sms value from various possible locations in Vapi's analysis
function extractAgreedToSms(msg) {
  const analysis = msg.analysis || (msg.artifact && msg.artifact.analysis) || {};
  const sd = analysis.structuredData || analysis.structured_data || analysis.structuredOutputs || analysis.structured_outputs || {};

  // Format 1: direct field { agreed_to_sms: true }
  if (typeof sd.agreed_to_sms === 'boolean') return sd.agreed_to_sms;
  if (typeof sd.agreedToSms === 'boolean') return sd.agreedToSms;
  if (sd.agreed_to_sms === 'true') return true;
  if (sd.agreed_to_sms === 'false') return false;

  // Format 2: UUID-keyed { "uuid": { name: "agreed_to_sms", result: true } }
  for (const key in sd) {
    const item = sd[key];
    if (item && typeof item === 'object' && (item.name === 'agreed_to_sms' || item.name === 'agredd_to_sms')) {
  ...
  // Format 3: top-level boolean
  if (typeof sd === 'boolean') return sd;
  return null;
}

app.post('/webhooks/vapi', express.json({ limit: '5mb' }), async function(req, res) {
  res.status(200).send('ok');
  ...
  if (msg.type === 'status-update') {
    ...
    if (reasonLower.indexOf('voicemail') >= 0) {
      ...
      const agreed = extractAgreedToSms(msg);
      rec.agreedToSms = agreed;
      if (agreed === true) {
        smsReason = 'customer_agreed';
      } else if (agreed === false) {
        addEvent(rec, 'sms_skipped', 'customer declined');
      } else {
        addEvent(rec, 'sms_skipped', 'agreement unclear, defaulting to skip');
      }
    } else {
    ...
    if (smsReason && rec.status !== 'recovered_after_call') {
      await maybeSendSms(rec, smsReason);
    }
  }
});
...
function statsHandler(req, res) {
  ...
  const c = {
    ...
    sms_sent: todayRecords.filter(function(r) { return r.smsSent; }).length,
...
app.listen(PORT, function() {
  ...
  console.log('Twilio: ' + ((TWILIO_ACCOUNT_SID && TWILIO_AUTH_TOKEN && TWILIO_FROM_NUMBER) ? 'ready (from ' + TWILIO_FROM_NUMBER + ')' : 'NOT configured'));
